...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!resendApiKey || !fromEmail) throw new Error("Falta configurar el correo saliente.");
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${resendApiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: fromEmail, to: booking.email, subject, text }) });
    if (!emailResponse.ok) {
